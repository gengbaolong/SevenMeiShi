...
public class NameTouchMovementMethod extends LinkMovementMethod {
    ...
    public void newClickTask(ClickTask task){
        removeAllTask();
        tasks.add(task);
    ...
    private int getOffsetForHorizontal(TextView widget, MotionEvent event){
    ...
    private void setSpanBackgroundGrayEffect(ClickableSpan span, Spannable buffer){
        buffer.setSpan(new BackgroundColorSpan(Color.GRAY),buffer.getSpanStart(span),buffer.getSpanEnd(span), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    }
    ...
    public boolean onTouchEvent(TextView widget, Spannable buffer, MotionEvent event) {
        ...
        switch (event.getAction()){
            case MotionEvent.ACTION_DOWN:
                setSpanBackgroundGrayEffect(span, buffer);
